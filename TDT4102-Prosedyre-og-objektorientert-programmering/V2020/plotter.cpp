...
Color::Color_type GraphColor::next_color()
{
  ...
  if(cur_color==colors.size()){
    throw "Error: No more colors accessible.";
  }
...
GraphColorRandom::GraphColorRandom()
{
  ...
  for(unsigned int i{0}; i<colors.size()-2;++i){
    unsigned int j = i+rand()%(colors.size()-i);
    Color::Color_type temp=colors[j];
    colors[j]=colors[i];
    colors[i]=temp;
  }
                         ...
                         Dam::WaterAlarm alarm)
{
  ...
  if(alarm!=Dam::WaterAlarm::NoAlarm){
    Line* temp= new Line{Point{calc_x(timestep), y_len + 40},Point{calc_x(timestep), y_len + 60}};
    temp->set_color(get_color(dam));
    attach(*temp);
...
{
  // BEGIN: P6
    Line* temp = new Line{Point{calc_x(0),calc_y(limit)},Point{calc_x(timesteps-1),calc_y(limit)}};
    temp->set_color(get_color(dam));
    temp->set_style(Line_style::dash);
    attach(*temp);
    shapes.push_back(temp);
  // END: P6
}
