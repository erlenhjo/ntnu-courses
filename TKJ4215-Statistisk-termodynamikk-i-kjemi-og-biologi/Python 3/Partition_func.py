...
def Calculate_Q_exact(m_AB, T):
    Boltzmann_factors = [exp(-E/T) for E in m_AB]
    #Should return a list of all Boltzmann factors (e^(-E/T))
    return sum(Boltzmann_factors)
    #The function return the exact partition function, as a sum of all Boltzmann factors.

def Calculate_Q_approx(Number_of_particles, T):
    W = factorial(Number_of_particles)/(factorial(Number_of_particles/2)**2) 
    #Should return the multiplicity of a system with N particle and N_A = N_B = N/2
    return W*exp(-Number_of_particles/T)
    #The function should return the Bragg-Williams approximated partition function
...
def Partition_function_plot(m_AB, Number_of_particles): #This is the plotting function. You do not need to do anything to this function.
    ...
    Q_exact_list = [Calculate_Q_exact(m_AB, T) for T in Temp]
    Q_approx_list = [Calculate_Q_approx(Number_of_particles, T) for T in Temp]
    Q_ratio = [Q_exact_list[i]/Q_approx_list[i] for i in range(len(Temp))]
